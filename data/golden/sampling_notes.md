...
Time: ~5.5 hours over one day, split into three sessions
...
(5/200). I considered keyword-filtering to force more in but decided
against it, because I wanted the class counts to reflect the real inbox
shape.

`other` initially came out high (53/200) because I kept mid-thread
fragments, non-English tweets, and hostile one-liners in the sample rather
than filtering them: the escalation path needs to see these in eval.
Final count is 33/200 after the intent broadening described in the
addendum below.
...
- Non-English tweets. Tagalog, Portuguese, Thai, French, and Indonesian
...
**gold_intent**: pick the most specific intent that fits. When two fit
(e.g. `subscription_billing` vs `cancel_refund` on money-related tweets,
or `account_management` vs `login_access` on password-reset cases), I
went with the more actionable one and noted the alternative in the
`notes` field.

**gold_reply_key_points**: 3-5 plain-English bullets. Not Spotify's
voice; those come from the RAG drafter at run time. These bullets just
describe what the reply must *cover*, not how it should sound. Example
for a billing dispute: `"acknowledge the charge, ask for last 4 of card,
...
**escalation_reason_if_yes**: short phrase, not a paragraph. Something a
routing agent can glance at.
...
A few labels I'd probably revise on a second pass:

- `#2317499` "downloads gone after update", labelled
  `content_availability`. You could argue `playback_bug`. I kept
  `content_availability` because the state issue is about the library,
  not the audio pipeline.
- `#567184` "family plan sharing policy is bonkers", labelled
  `feedback` (policy feedback bucket). Could equally be
  `account_management`.
- `#2029474` "you removed Samsung TV app, will you discount?",
  labelled `cancel_refund` because the customer is asking for a
  discount, but the trigger is a feature removal.

None of these change the escalation call, so I left them as-is. They
would be interesting rows to look at when reading the confusion matrix
in `eval/results/confusion.json`.
...
`other` (33/200 in the final count) whenever they didn't map cleanly to
one of the six "specific problem" buckets. I broadened all 8 definitions
in `src/intents.py`. The most important change was renaming
`praise_feedback` to `feedback` and widening it to include *criticism*
and *feature requests*, not just praise. That brought ~20 golden entries
out of `other` and into the specific intents they actually belonged in
...
- `cancel_refund` is 5/200. Per-class F1 on refund will be noisy: one
  bad prediction moves it 20 points. I flag this in the report.
